...
export async function buildTanakh(): Promise<Node[]> {
  ...
  const cats = [{ api:'Torah', id:'tanakh:torah', he:'תורה' }, { api:'Prophets', id:'tanakh:neviim', he:'נביאים' }, { api:'Writings', id:'tanakh:ketuvim', he:'כתובים' }]
  for (let i=0;i<cats.length;i++){
    const cat = cats[i]
    nodes.push({ id:cat.id, title:cat.he, kind:'card', color:'blue', parentId:'tanakh', order:i+1 })
    const list:any = await j(`${BASE}/category/${cat.api}`)
    const books = Array.isArray(list.books) ? list.books as string[] : []
    let order = 1
    for (const b of books) {
      const meta:any = await j(`${BASE}/index/${encodeURIComponent(b)}`)
      const bid = `${cat.id}:${b.replace(/\s+/g,'_')}`
      nodes.push({ id: bid, title: meta.heTitle ?? b, kind:'card', color:'blue', parentId:cat.id, order: order++ })
      const lengths = meta.lengths || meta?.schema?.lengths || []
      const chapCount = Array.isArray(lengths) ? (lengths[0] || 0) : 0
      for (let c=1; c<=chapCount; c++){ nodes.push({ id:`${bid}:${c}`, title:`פרק ${toHebNum(c)}`, kind:'check', parentId: bid, order:c }) }
    }
  }
  return nodes
}
...
export async function buildMishnah(): Promise<Node[]> {
  ...
  const orders:any = await j(`${BASE}/category/Mishnah`)
  let o=1
  for (const order of A<any>(orders.contents)) {
    const orderId = `mishnah:${(order.category || order.title || 'order').replace(/\s+/g,'_')}`
    nodes.push({id:orderId, title:(order.heCategory || order.heTitle || order.he || order.title), kind:'card', color:'brown', parentId:'mishnah', order:o++})
    let t=1
    for (const m of A<any>(order.contents)) {
      if (!m.title) continue
      const meta:any = await j(`${BASE}/index/${encodeURIComponent(m.title)}`)
      const mId = `${orderId}:${m.title.replace(/\s+/g,'_')}`
      nodes.push({id:mId, title:(meta.heTitle || m.title), kind:'card', color:'brown', parentId:orderId, order:t++})
      ...
      for (let p=1;p<=perakim;p++){
        ...
        const chapMeta:any = await j(`${BASE}/texts/${encodeURIComponent(m.title)}.${p}?lang=he`)
        const mishCount = Array.isArray(chapMeta.he) ? chapMeta.he.length : 0
        for (let mi=1;mi<=mishCount;mi++){ nodes.push({id:`${pId}:${mi}`, title:`משנה ${toHebNum(mi)}`, kind:'check', parentId:pId, order:mi}) }
      }
    }
  }
  return nodes
...
const BAVLI_ORDER: Record<string, string> = { 'Berakhot':'Zeraim','Shabbat':'Moed','Eruvin':'Moed','Pesachim':'Moed','Shekalim':'Moed','Rosh Hashanah':'Moed','Yoma':'Moed','Sukkah':'Moed','Beitzah':'Moed','Taanit':'Moed','Megillah':'Moed','Moed Katan':'Moed','Chagigah':'Moed','Yevamot':'Nashim','Ketubot':'Nashim','Nedarim':'Nashim','Nazir':'Nashim','Sotah':'Nashim','Gittin':'Nashim','Kiddushin':'Nashim','Niddah':'Taharot','Bava Kamma':'Nezikin','Bava Metzia':'Nezikin','Bava Batra':'Nezikin','Sanhedrin':'Nezikin','Makkot':'Nezikin','Shevuot':'Nezikin','Avodah Zarah':'Nezikin','Horayot':'Nezikin','Zevachim':'Kodashim','Menachot':'Kodashim','Chullin':'Kodashim','Bekhorot':'Kodashim','Arakhin':'Kodashim','Temurah':'Kodashim','Keritot':'Kodashim','Meilah':'Kodashim','Tamid':'Kodashim','Middot':'Kodashim' }
...
export async function buildBavli(): Promise<Node[]> {
  ...
  const cat:any = await j(`${BASE}/category/Talmud`)
  ...
  for (const t of A<any>(bavli.contents)) {
    if (!t.title) continue
    const meta:any = await j(`${BASE}/index/${encodeURIComponent(t.title)}`)
    const eng = meta.title as string
    const parentId = `gemara:${(BAVLI_ORDER[eng] || 'Moed').toLowerCase()}`
    counters[parentId] = (counters[parentId] ?? 0) + 1
    const mId = `${parentId}:${eng.replace(/\s+/g,'_')}`
    nodes.push({id:mId, title:(meta.heTitle || eng), kind:'card', color:'brown', parentId, order:counters[parentId]})
    const dafCount = (Array.isArray(meta.lengths) && meta.lengths[0]) || 0
    for (let d=2; d<=dafCount+1; d++){ nodes.push({id:`${mId}:${d}`, title:`דף ${d===2?'ב':toHebNum(d)}`, kind:'check', parentId:mId, order:d-1}) }
  }
  return nodes
}
...
export async function buildYerushalmi(): Promise<Node[]> {
  ...
  const cat:any = await j(`${BASE}/category/Jerusalem_Talmud`)
  let i=1
  for (const t of A<any>(cat.contents)) {
    if (!t.title) continue
    const meta:any = await j(`${BASE}/index/${encodeURIComponent(t.title)}`)
    const mId = `yeru:${t.title.replace(/\s+/g,'_')}`
    nodes.push({id:mId, title:(meta.heTitle || t.title), kind:'card', color:'grey', parentId:'yeru', order:i++})
    const perakim = (Array.isArray(meta.lengths) && meta.lengths[0]) || 0
    for (let p=1;p<=perakim;p++){
      const pId = `${mId}:${p}`
      nodes.push({id:pId, title:`פרק ${toHebNum(p)}`, kind:'card', color:'grey', parentId:mId, order:p})
      const chapMeta:any = await j(`${BASE}/texts/${encodeURIComponent(t.title)}.${p}?lang=he`)
      const sugyaCount = Array.isArray(chapMeta.he) ? chapMeta.he.length : 0
      for (let s=1;s<=sugyaCount;s++){ nodes.push({id:`${pId}:${s}`, title:`סוגיה ${toHebNum(s)}`, kind:'check', parentId:pId, order:s}) }
    }
  }
  return nodes
}
...
export async function buildShulchanAruch(): Promise<Node[]> {
  ...
  for (const part of SA_PARTS) {
    ...
    const meta:any = await j(`${BASE}/index/${encodeURIComponent(part.eng)}`)
    ...
    for (let s=1;s<=simanim;s++){
      ...
      const chap:any = await j(`${BASE}/texts/${encodeURIComponent(part.eng)}.${s}?lang=he`)
      const seifCount = Array.isArray(chap.he) ? chap.he.length : 0
      for (let se=1; se<=seifCount; se++){ nodes.push({id:`${simanId}:${se}`, title:`סעיף ${toHebNum(se)}`, kind:'check', parentId:simanId, order:se}) }
    }
  }
  return nodes
}
...
export async function buildRambam(): Promise<Node[]> {
  ...
  const root:any = await j(`${BASE}/category/Mishneh_Torah`)
  let b=1
  for (const book of A<any>(root.contents)) {
    ...
    for (const hil of A<any>(book.contents)) {
      ...
      const meta:any = await j(`${BASE}/index/${encodeURIComponent(hil.title)}`)
      const hilId = `${bookId}:${hil.title.replace(/\s+/g,'_')}`
      nodes.push({id:hilId, title:(meta.heTitle || hil.title), kind:'card', color:'grey', parentId:bookId, order:h++})
      ...
      for (let p=1;p<=chapters;p++){
        ...
        const chap:any = await j(`${BASE}/texts/${encodeURIComponent(hil.title)}.${p}?lang=he`)
        const halCount = Array.isArray(chap.he) ? chap.he.length : 0
        for (let ha=1; ha<=halCount; ha++){ nodes.push({id:`${pId}:${ha}`, title:`הלכה ${toHebNum(ha)}`, kind:'check', parentId:pId, order:ha}) }
      }
    }
  }
  return nodes
...
export async function buildEverything(): Promise<Node[]> {
  const out: Node[] = [{id:'home', title:'ספרים', kind:'card', order:0}]
  async function graft(title:string, color:'blue'|'brown'|'orange'|'grey', prefix:string, fn:()=>Promise<Node[]>, order:number) {
    try {
      ...
      for (const n of arr) {
        if (n.id==='home' || n.id===prefix) continue
        const nn: Node = { ...n }
      ...
      console.error('IMPORT ERROR', prefix, e?.message)
    }
  }
